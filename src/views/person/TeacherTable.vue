  ...
  <div class="base_form">
    ...
    <div class="base_query_oneLine">
      ...
      <div class="query_right">
        <el-input v-model="numName">工号或姓名</el-input>
    ...
    <div class="table-content">
      <el-table :data="dataList" :header-cell-style="{
        ...
        <el-table-column label="地址" color="black" align="center">
          <template v-slot="scope">
            {{ scope.row.person.address }}
          </template>
        </el-table-column>
        <el-table-column label="操作" color="black" align="center" width="230">
          <template v-slot="scope">
            <el-button class="commButton" @click="editItem(scope.row.personId)">编辑</el-button>
            <el-button class="commButton" @click="deleteItem(scope.$index)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    //添加教师,跳转到教师信息页面
    addItem() {
      router.push({ path: "/TeacherInfo" });
    },
    //编辑教师,跳转到教师信息页面
    editItem(personId: number) {
      router.push({
        path: "/TeacherInfo",
    ...
    async deleteItem(index: number) {
      ...
      const res = await teacherDelete(this.dataList[index].personId);
      if (res.code == 0) {
        message(this, "删除成功");
