  ...
  <div class="base_form">
    ...
    <div class="base_query_oneLine" style="width: 97%">
      ...
      <div class="query_left">
        ...
        <el-button class="commButton" type="success" @click="handleExport()" style="margin-left: 10px;">导出数据</el-button>
      </div>
      <!-- 右边查询组 -->
      <div class="query_right">
        <span style="margin-top: 5px">状态</span>
        <el-select v-model="searchParams.state" placeholder="选择状态" style="width: 120px; margin-left: 10px; margin-right: 10px;">
          <el-option
    ...
    <div class="form-div" style="margin-top: 5px">
      <el-form :model="form" :rules="rules" ref="formRef" label-position="right" label-width="100px">
          ...
          >
            ...
            <template #tip>
              <div class="el-upload__tip">
                支持上传 PDF, DOC, DOCX, JPG, PNG 格式文件
              </div>
            </template>
          </el-upload>
          <el-button type="success" size="small" @click="uploadFile" style="margin-top: 10px;" :disabled="!fileList.length" :loading="isUploading">
            上传附件
          </el-button>
        ...
        <el-form-item>
          ...
          <el-button type="primary" @click="submitForm()" style="margin-left: 10px;" :loading="loading.submitForm">提交</el-button>
        </el-form-item>
      </el-form>
    </div>
    <!-- 数据表格区域 -->
    <div class="table-content" style="margin-top: 20px">
      <div class="batch-operations" style="margin-bottom: 10px;">
        <el-button size="small" class="commButton" type="primary" @click="batchApprove" :disabled="selectedRows.length === 0" :loading="loading.batchCheck">批量批准</el-button>
        <el-button size="small" class="commButton" type="danger" @click="batchReject" :disabled="selectedRows.length === 0" :loading="loading.batchCheck">批量拒绝</el-button>
        <span style="margin-left: 10px; color: #999;">{{ selectedRows.length }} 项已选择</span>
      </div>
      <el-table :data="leaveList" :header-cell-style="{
        color: '#2E2E2E',
        fontSize: '10px',
        fontWeight: '400',
        background: 'rgb(242,242,242)',
      }" :row-style="{ height: '10px' }" :cell-style="{ padding: '2px' }" style="width: 100%" :loading="loading.leaveList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" color="black" align="center" width="200" fixed="right">
          <template v-slot="scope">
            ...
            <el-button size="small" class="commButton" type="primary" @click="checkItem(scope.row, 1)" v-if="scope.row.state === 0">批准</el-button>
            <el-button size="small" class="commButton" type="danger" @click="checkItem(scope.row, 2)" v-if="scope.row.state === 0">拒绝</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
import { getStudentLeaveList, studentLeaveSave, studentLeaveCheck, getTeacherItemOptionList, uploadAttachment, exportLeaveList, type TeacherOption, type LeaveItem, type LeaveSearchParams, type LeaveSaveRequest } from '~/services/studentLeaveServ';
import type { UploadFile } from 'element-plus';

// ========== 响应式变量定义（指定类型） ==========
// 表单数据类型（基于LeaveSaveRequest扩展）
...
// 请假列表数据（指定LeaveItem数组类型）
const leaveList = ref<LeaveItem[]>([]);

// 教师选项列表（核心修复：指定TeacherOption数组类型）
const teacherOptions = ref<TeacherOption[]>([]);

// 状态选项（指定StateOption数组类型）
...
// 搜索条件（使用服务层定义的LeaveSearchParams类型）
...
const loadTeacherOptions = async () => {
  ...
  try {
    ...
    // 确保返回数据符合TeacherOption类型（如果接口返回格式不一致，需转换）
    if (Array.isArray(res)) {
      // 转换数据格式确保符合TeacherOption类型
      teacherOptions.value = res.map(item => {
        // 使用类型断言处理API可能返回的不同字段名
        const apiItem = item as any;
        return {
          label: apiItem.label || apiItem.teacherName || '未知教师',
          value: Number(apiItem.value) || Number(apiItem.teacherId) || 0
        };
      });
    } else {
      // 如果不是数组，初始化为空数组
      teacherOptions.value = [];
      console.error('教师选项数据格式错误，不是数组:', res);
    }
  } catch (error) {
...
const loadLeaveList = async () => {
  ...
  try {
    ...
    leaveList.value = res as LeaveItem[];
  } catch (error) {
...
const handleDateRangeChange = () => {
  // 日期范围变化时更新表单字段
  if (dateRange.value) {
    form.value.leaveStartDate = dateRange.value[0];
    form.value.leaveEndDate = dateRange.value[1];
  } else {
...
const submitForm = async () => {
  ...
  try {
    ...
    // 提交数据
    await studentLeaveSave(form.value);
...
const handleExport = async () => {
  try {
    ...
    // 错误处理已在downloadPost函数中完成，这里可以不再重复提示
  }
};
  ...
  try {
    ...
    const newNotifications = res.filter(item => {
      ...
      return item.state !== 0 && 
             ...
             (currentTime - lastCheckTime < 300000); // 5分钟内的新审核
    });
