    ...
    <div class="table-content">
      <el-table :data="dataList" :header-cell-style="{
        ...
        <el-table-column label="操作" width="150" color="black" align="center">
          <template v-slot="scope">
            <el-button class="commButton" @click="saveItem(scope.$index)">保存</el-button>
...
export default defineComponent({
  ...
  async created() {
    ...
    if (this.personId != null) {
      this.dataList = await getFamilyMemberList(this.personId);
    }
  },
  methods: {
    //在数据表中添加一行
    addRow() {
      this.dataList.push({ personId: this.personId } as FamilyMemberItem);
    },
    //保存数据表中的一行
